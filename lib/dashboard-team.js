...
.proj-pill {
  display: inline-block; padding: 2px 9px; border: 1px solid color-mix(in srgb, var(--accent) 16%, transparent); border-radius: 999px;
  color: var(--accent); background: color-mix(in srgb, var(--accent) 5%, transparent); font: 600 10px/1.6 var(--mono); cursor: pointer;
}
.proj-pill:hover { border-color: color-mix(in srgb, var(--accent) 40%, transparent); }
...
function renderTeam(d) {
  ...
  // Settings absorbs team management: refresh it from the same payload.
...
function renderCurrent() {
  // In Settings, re-render the management panels instead of the hub grid so
  // handlers that call renderCurrent() (e.g. revoke-invite) refresh Settings.
...
function handleTeamClick(e) {
  ...
  if (action === 'remove-member') {
    ...
    teamRequest('/api/team/remove-member', { teamId: teamSelId, userId: btn.dataset.userId }).then(function () {
      ...
      if (currentTab() !== 'settings') location.hash = '#team';
      loadTeam();
    }).catch(function (err) { setTeamNotice(err.message, 'error'); loadTeam(); });
    return;
  }
