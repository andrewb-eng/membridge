...
async function resolveOneShotCrypto(config, creds, link, opts = {}) {
  ...
  try {
    ...
    return cur ? { teamKey: cur.teamKey, epoch: cur.epoch, teamcrypto: deps.teamcrypto } : null;
  } catch (err) {
    ...
    try {
      ...
      if (keyDeps) {
        try {
          ...
          if (cur) {
            crypto = { teamKey: cur.teamKey, epoch: cur.epoch, teamcrypto: cryptoCtx.teamcrypto };
          } else {
