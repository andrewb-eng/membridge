...
async function pushProject(config, creds, projectPath, proj, link, crypto) {
  ...
  const rowFor = e => entryToRow(e, link.projectId, creds, isShared(config, proj, e.session), regexes);
...
async function pullProject(config, creds, proj, link, teamCrypto) {
  ...
  // (author, ts, source) key is by definition a NEWER version — the author
  // re-pushed it with a bumped created_at (drift re-push, reshare) — so it
  // REPLACES the stale stored copy instead of being dropped.
  const seen = new Map(existing.map((e, i) => [`${e.author}|${e.ts}|${e.source}`, i]));
  for (const r of rows) {
    ...
    if (seen.has(k)) {
      existing[seen.get(k)] = mapped;
    } else {
      seen.set(k, existing.length);
...
module.exports = {
  ...
  syncTeams, reshareSession, pullProject, credentialsPath, teamFilePath,
};
