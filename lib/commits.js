...
function readCommit(projectPath, sha, deps = {}) {
  ...
  try {
    const out = String(runGit(['show', '--numstat', '--format=%cI|%P', '--no-color', '--no-show-signature', sha, '--']));
    const head = (out.split('\n').find(l => l.trim()) || '').trim();
    const sep = head.indexOf('|');
    const ts = (sep === -1 ? head : head.slice(0, sep)).trim() || null;
    const parents = (sep === -1 ? '' : head.slice(sep + 1)).trim().split(/\s+/).filter(Boolean);
    if (parents.length >= 2) return { sha, ts, files: [] };
    return { sha, ts, files: [...parseNumstat(out).keys()] };
  } catch {
    return { sha, ts: null, files: [] };
  }
}
