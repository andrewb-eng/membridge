...
//
// Dedupe-by-sha (Task 3): the hook and the daemon backfill both append-only
// write this file, and both gate on "is this sha already recorded" before
// writing (see recordCommit callers in lib/hooks.js / lib/scan.js) — a race
// between them (or an authorship-gate answer that flips between the two
// writes because `git config user.email` changed in between) can still land
// two rows for the same sha. Reads must present exactly one. Preference,
// deliberately chosen over "first row wins" (which is what a naive scan
// would do by accident, and what lib/provenance.js's old `.find()` happened
// to do before this was a decided rule):
//   1. a row with a real (non-empty) session beats an empty-session row for
//      the same sha — an empty row only ever means "foreign/unattributable
//      commit" (the authorship gate, or the hook's stale-state defer), which
//      is strictly less informative than an attributed one, no matter which
//      writer produced it or in which order.
//   2. between two equally-informative rows (both empty or both real), the
//      LATER append wins — it reflects the freshest attribution pass (e.g. a
//      daemon run that saw more merged events than the hook did).
...
function loadCommitMap(projectPath) {
  ...
  for (const r of parseRawRecords(raw)) {
    ...
    const prevReal = Array.isArray(prev.sessions) && prev.sessions.length > 0;
    const curReal = Array.isArray(r.sessions) && r.sessions.length > 0;
    if (curReal || !prevReal) bySha.set(r.sha, r); // curReal wins outright; equal informativeness -> later wins
  }
