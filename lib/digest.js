...
function renderBlock(projectPath, proj, config, target) {
  ...
  if (target === 'AGENTS.md' && (!config || !config.distill || config.distill.enabled !== false)) {
    lines.push('When you complete a task here, append ONE line to `.membridge/summaries.jsonl`: `{"session":"<your session id>","ts":"<ISO time>","did":"<1-3 sentences on what you did>","decisions":"","gotchas":""}` — plain text, only what a teammate needs.');
    lines.push('');
  }
...
module.exports = {
  ...
  sessionGroups, todoCounts, pickSummary,
};
