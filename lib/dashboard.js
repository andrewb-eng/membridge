...
// loop, so one broken row cannot block the rest of the batch. Pure aside from
// the injected fetch-like dep — offline-testable, no DOM.
function deleteProjectsBulk(paths, fetchFn) {
  ...
  function next() {
    ...
    return fetchFn('/api/projects/delete', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ path: p }),
    }).then(function () { deleted.push(p); return next(); })
