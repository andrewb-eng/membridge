...
function submitAdd() {
  ...
  fetch('/api/projects/add', {
    ...
    if (res.ok) { closeAdd(); if (currentTab() === 'settings') loadProjectsSettings(); else loadOverview(); return; } // added or already tracked
...
function renderTeamSettings(d) {
  ...
  var teams = d.teams || [];
  if (!teams.length) { curTeam = null; host.innerHTML = noTeamSettingsHtml(d); return; }
  var sel = null;
...
function pjDisplayName(entries, value) {
...
function pjMenuHtml(detail, value) {
  var items = '';
  if (detail) {
    var p = detail.path;
...
function renderProject(feed, detail, value) {
  ...
  var name = local ? detail.name : pjDisplayName(entries, value);
  var chip = local
