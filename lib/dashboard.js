...
/* ---- slow-load spinner: shared by the Projects/Activity/Team loaders. A
   ...
   settling late fails the token check in both armSpinner's own timer and
   clearSpinner, so it can never hide or cancel a newer load's spinner. ---- */
var SPINNER_DELAY_MS = 3000;
var spinnerTokens = {}, spinnerTimers = {};
...
function armSpinner(viewKey, hostId) {
  ...
  spinnerTimers[viewKey] = setTimeout(function () {
    ...
    if (host) host.innerHTML = spinnerHtml();
  }, SPINNER_DELAY_MS);
  return token;
}
function clearSpinner(viewKey, token) {
  if (spinnerTokens[viewKey] !== token) return; // a newer load owns this view now
  if (spinnerTimers[viewKey]) { clearTimeout(spinnerTimers[viewKey]); spinnerTimers[viewKey] = null; }
}
...
function loadProjectsIndex() {
  ...
  var pxLoadToken = armSpinner('projects', 'homeCatchup');
  fetch('/api/projects').then(function (r) { return r.json(); }).then(function (projects) {
    projects = projects || [];
    if (!projects.length) { pxData = null; pxFp = ''; renderProjectsEmpty(host); clearSpinner('projects', pxLoadToken); return; }
    ...
    Promise.all([
      ...
      if (fp === pxFp && host.firstChild && (pxMenuId || pxConfirmId)) { refreshAgo('view-home'); clearSpinner('projects', pxLoadToken); return; }
      if (fp === pxFp && host.firstChild) { refreshAgo('view-home'); clearSpinner('projects', pxLoadToken); return; }
      pxFp = fp;
      renderProjectsIndex();
      clearSpinner('projects', pxLoadToken);
...
function loadFeed() {
  ...
  fetch(feedViewUrl()).then(function (r) { return r.json(); }).then(function (f) {
    ...
    if (fp === feedFp && host.firstChild) { refreshAgo('view-feed'); clearSpinner('feed', feedLoadToken); return; }
    feedFp = fp;
    renderFeed(host, entries);
    clearSpinner('feed', feedLoadToken);
...
function feedFilterBarHtml(entries) {
  entries.forEach(function (e) {
    if (e.project) feedProjectsSeen[e.project] = e.project;
  ...
  var projOpts = [all].concat(Object.keys(feedProjectsSeen).sort().map(function (k) { return { label: feedProjectsSeen[k], val: k }; }));
