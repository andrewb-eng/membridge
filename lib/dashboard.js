...
function threadHtml(t) {
  ...
  var seeAll = n > 3
    ...
    : '';
  // Changes, mirroring feedEntryHtml's detail: only when the rep entry carries
  // a changes list, kept inside the dropdown so a collapsed card stays compact.
  var changesBlock = (t.rep && Array.isArray(t.rep.changes) && t.rep.changes.length)
    ? '<div class="fd-label">Changes</div>' + changesHtml(t.rep.changes)
    : '';
  var kidsBlock = '<div style="display:' + (open ? 'block' : 'none') + '">' +
    '<div style="margin:2px 0 18px 13px;padding:2px 0 2px 25px;border-left:2px solid var(--border)">' + changesBlock + kids + seeAll + '</div></div>';
