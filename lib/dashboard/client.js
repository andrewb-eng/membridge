...
/* ===================== The Projects index (#home) =====
   The default landing: a compact scan grid (design 1c) — one row per project
   with name+badge, the latest session summary, an avatar stack of recent
   people, a 24-hour sparkline and the ⋯ menu, so ~15 projects read at a
   glance. Paused/dormant projects dim to a single status line. "N new" is
   computed from the GLOBAL catch-up pointer (/api/catchup lastViewedTs)
   against the recent feed — the only read-state we have. Filters/sort are
   client-side; the row ⋯ menu pauses/resumes and deletes (role-gated:
   shared+manager archives for the team, a member unlinks their machine,
   local deletes). Poll-deduped via pxFp so an open menu survives the 5s
   poll. */
...
// Pure (offline-testable, pxPersonFilter style — deps injected): the newest
// recent-feed entry for a project, the "Latest session" cell. Picks by ts,
// not array order, so it is robust to whatever order the feed pages in.
function pxLatestEntry(p, recent, entryInProject) {
  var best = null;
  (recent || []).forEach(function (e) {
    if (!entryInProject(e, p)) return;
    if (!best || String(e.ts || '') > String(best.ts || '')) best = e;
  });
  return best;
}
// Pure: sessions-per-hour over the trailing 24 hours (oldest bucket first),
// counted from the recent feed window. The clock is injected (now) so tests
// can pin it.
function pxSparkCounts(p, recent, entryInProject, now) {
  var HOUR = 3600000;
  var counts = [];
  for (var i = 0; i < 24; i++) counts.push(0);
  (recent || []).forEach(function (e) {
    if (!entryInProject(e, p)) return;
    var t = new Date(e.ts).getTime();
    if (isNaN(t)) return;
    var back = Math.floor((now - t) / HOUR);
    if (back >= 0 && back < 24) counts[23 - back]++;
  });
  return counts;
}
// Pure: cap the latest-session summary at a fixed length, cutting on a word
// boundary. The full text stays in the cell's title attribute, so hovering
// always renders the whole summary.
function pxClipSummary(s, max) {
  s = String(s || '');
  if (s.length <= max) return s;
  var cut = s.slice(0, max).replace(/\\s+\\S*$/, '');
  return (cut || s.slice(0, max)) + '…';
}
var PX_SUMMARY_MAX = 140;
// Pure: distinct people on a project's recent entries, newest first, capped
// to what the avatar stack can show.
function pxWhoList(p, recent, entryInProject) {
  var seen = {}, out = [];
  var rows = (recent || []).slice().sort(function (a, b) { return String(b.ts || '').localeCompare(String(a.ts || '')); });
  rows.forEach(function (e) {
    if (out.length >= 3 || !entryInProject(e, p)) return;
    var name = e.self ? 'You' : (e.author || '');
    if (!name) return;
    var key = e.authorId || name;
    if (seen[key]) return;
    seen[key] = 1;
...
function pxInitials(name) {
  var w = String(name).trim().split(/\\s+/);
  return (w.length > 1 ? w[0][0] + w[1][0] : w[0].charAt(0)).toUpperCase();
}
// A project quiet this long collapses to a dim one-line row.
var PX_DORMANT_DAYS = 21;
// One project row — the 1c compact-scan grid: name+badge | latest session |
// who | 24h sparkline | ⋯. Same data-px-* hooks as the old editorial rows,
// so the delegated listener and the menu poll-survival logic are unchanged.
function pxRowHtml(p) {
  ...
  var menu = '';
  if (menuOpen) {
    menu = '<div data-px-menu-pop style="position:absolute;right:4px;top:44px;width:220px;background:var(--card);border:1px solid var(--border);border-radius:16px;box-shadow:var(--shadow-xl);padding:6px;z-index:50;animation:mbFade .2s cubic-bezier(.16,1,.3,1)">' +
  ...
  var badge = p.team ? ((p.team.teamName || '').trim() || 'team') : 'local';
  var nameCell = '<div class="px-cell" data-px-open="' + pathAttr + '" title="' + pathAttr + '">' +
    '<div class="px-name">' + (newN > 0 ? '<span class="px-dot"></span>' : '') + '<b>' + esc(p.name) + '</b></div>' +
    '<div class="px-sub' + (p.team ? ' team' : '') + '">' + esc(badge) + (newN > 0 ? ' · ' + newN + ' new' : '') + '</div></div>';
  var menuCell = '<button class="px-menu-btn" data-px-menu="' + pathAttr + '">⋯</button>';

  var ageDays = p.lastActivity ? Math.floor((Date.now() - new Date(p.lastActivity).getTime()) / 86400000) : null;

  // Paused, never-active and dormant projects: one dim status line, no
  // summary/who/sparkline — they earn their space back when work resumes.
  if (p.paused || ageDays === null || ageDays >= PX_DORMANT_DAYS) {
    var when = p.lastActivity
      ? ' · <span data-ago="' + esc(p.lastActivity) + '">' + esc(ago(p.lastActivity)) + '</span>'
      : '';
    var status = p.paused
      ? '<span class="px-chip">paused</span><span>new sessions aren&rsquo;t being captured' + when + '</span>'
      : ageDays === null
        ? '<span>no activity yet</span>'
        : '<span>dormant &mdash; quiet for ' + Math.floor(ageDays / 7) + ' weeks</span>';
    return '<div class="px-row dim">' + nameCell +
      '<div class="px-cell px-status" data-px-open="' + pathAttr + '">' + status + '</div>' +
      '<span></span><span></span>' + menuCell + menu + '</div>';
  }

  var latest = pxLatestEntry(p, pxData.recent, pxEntryInProject);
  var quiet = newN === 0 && ageDays >= 3;
  ...
  if (latest) mparts.push(esc(latest.self ? 'You' : (latest.author || 'Someone')));
  if (latest && latest.source) mparts.push(esc(latest.source));
  var ts = (latest && latest.ts) || p.lastActivity;
  if (ts) mparts.push('<span data-ago="' + esc(ts) + '">' + esc(ago(ts)) + '</span>');
  ...
  var whoCell = '<div class="px-who">' + pxWhoList(p, pxData.recent, pxEntryInProject).map(function (w) {
  ...
  var spark = '<div class="px-spark" title="Sessions, last 24 hours">' + pxSparkCounts(p, pxData.recent, pxEntryInProject, Date.now()).map(function (v) {
...
function renderProjectsIndex() {
  ...
  // 1c footer: the create-a-team nudge moves under the grid as one quiet line.
  var footer = solo
    ? '<div class="px-foot">Working with others? <span data-px-goteam>Create a team</span> to share project memory.</div>'
    : '';
  var grid =
